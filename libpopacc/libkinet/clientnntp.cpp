...
ClientNNTP::command (const char * cmd, const char * av)
{
	...
	for(int retry = 0; retry < 5; ++retry)
	{
		...
		if ((strcasecmp(cmd, "GROUP") != 0) && (m_cGroup.length() > 0))
		{
		    ...
		    char *p = command("GROUP", m_cGroup.c_str());
		    if ((p == NULL) || (strncmp(p, "211 ", 4) != 0))
		    {
			DMSG(0, "ClientNNTP: Resend GROUP failed");
			return NULL;
		    }
	    ...
	    DMSG(0, "ClientNNTP: command %s %s: error: %s... retrying %d",
		cmd, (av?av:"(null)"), (p?p:"(null)"), retry+1);
	    Close();
	    sleep(1);
