...
//
// This should be merge into newsov.cpp someday.
//

// p -> Byte field where the OVExt
//
// Syntax:  <ByteSize> [ : <Type-1 digit> [ . <Partno> [ /<TotalPart> ] ] ]
//
const char *
OvExtParse (const char * p, int * pNum, int * pPart)
{
	char * pType = strchr(p, ':');
	if ((pType != NULL) && (atoi(pType+1) != 1))
	{
		if (pType[2] == '.')
		{
			if (pNum)
...
// flag = 1 => keep <> for ppID 
// return:
//	type == -1	-> error
//	type == 0	-> unknown
//	type == 1	-> text
//	type == 2	-> binary - 1st part
//	type == 3	-> binary - no 1st part
//
int
OVParseT (char * p, char ** ppArtno, char ** ppSubj,
	char ** ppAuthor, char ** ppID, char ** ppSize,
	int * pNum, int * pPart,
	char ** ppDate, char ** ppLine,
	int flag)
{
	int type = 0;
	char * q = p;
...
// 1st = Article Number
	...
	*ppSize = p;
	...
	if (pNum || pPart)
	{
		const char * pType = OvExtParse(p, pNum, pPart);
		if (pType)
		{
			type = pType[1] - '0';
		}
...
#else
	//
	// For a while OV may missing line field - temporary fake a line = 10
	// This can be remove in 90 days - kachun 20070113
	//
	{
		static char line10 [] = "10";
		if (ppLine)
			*ppLine = line10;
		return type;
	}
...
bool
OVParse (char * p, char ** ppArtno, char ** ppSubj,
	char ** ppAuthor, char ** ppID, char ** ppSize,
	int * pNum, int * pPart,
	char ** ppDate, char ** ppLine,
	int flag)
{
	if (OVParseT(p, ppArtno, ppSubj, ppAuthor, ppID, ppSize,
		pNum, pPart, ppDate, ppLine, flag) < 0)
	{
		DMSG(0, "Bad OV: %s", p);
		return false;
	}
...
#define BINARY_SIZE 5000

//
// Return exttype | 0x8000 == Re | 0x4000 = BINARC
//
POST_DEF
ParsePostDef (const char * pSubj, const char * pOvByte, bool bBinCol,
	ZString &subjroot, ZString &filenum)
{
	POST_DEF def;
	memset((void*) &def, 0, sizeof(POST_DEF));

	def.byte = atoi(pOvByte);
	bool bRe = (strncasecmp(pSubj, "Re:", 3) == 0);

	def.flag = (bRe ? POST_DEF_RE : 0) ;
	int reoff = 0;
	if(bRe)
	{
		const char * osubj = pSubj;
		for(pSubj += 3; isspace(*pSubj); pSubj++) ;
		reoff = pSubj - osubj;

#ifdef BINARY_SIZE
		if (def.byte < BINARY_SIZE)
		{
			subjroot = pSubj;
			return def;
		}
	...
	int iPart = 0;
	int iNum = 0;
	const char * pType = OvExtParse(pOvByte, &iNum, &iPart);
	if (pType != NULL)
		DMSG(2, "%s %s %d/%d", pSubj, pType, iNum, iPart);

	bool bText = (pType && (atoi(pType+1) == 1));
	NewsSubject subj(pSubj);
	int extpos = -1;
	def.exttype = subj.GetFileExt(&extpos);
	def.extpos = extpos;
	if (extpos > 0)
		def.extpos += reoff;
	...
	int nNum = 0;
	if((!bText || (iPart > 1)) &&
		subj.GetMultiSortSubj(subjroot, &nPart, &nNum,
			iPart, iNum, (def.exttype==EXT_COMB)?1:2,
			def.exttype, extpos, &filenum))
	{
		//
		// Limit to 32K parts
		//
		def.part_no = nNum;
		def.part_total = nPart;

		DMSG(2, "Multi->%s %d/%d (%s)", subjroot.c_str(),
			def.part_no, def.part_total, filenum.chars());

		def.flag |= POST_DEF_MULTI;
		return def;
	}

	if((def.exttype != EXT_UNKNOWN)
		&& (def.exttype != EXT_TXTENC) && (def.exttype != EXT_COMB)
		&& !bText)
	{
		char buf[subj.stg().length()+1];
		subj.GetBinArcSubj(buf, &filenum,
			true, def.exttype, extpos);
		subjroot = buf;

		DMSG(2, "%s\n%s %s", pSubj, buf, filenum.chars());
		def.flag |= POST_DEF_BINARC;
		return def;
	}
