...
ClientNNTP::groupcmd (const char * grp, grpinfo &gi)
{
	...
	for (retry = 0; retry < MAXRETRY; retry++)
	{
		...
		if (p &&  (strncmp(p, "411 No Such Group", 17) == 0))
			return false;

		if ((p == NULL) || (strncmp(p, "211 ", 4) != 0))
		{
	        	DMSG(0, "ClientNNTP: %s groupcmd error: %s %s.. retrying %d",
				m_pNNTP->GetConnInfo(), grp, p, retry+1);
			Close();
		...
		if (range >= minrange)
			...
			m_pNNTP->GetConnInfo(), grp, minrange, range, retry+1);

		// Keep the FE with more articles
	...
	if (retry >= MAXRETRY)
	{
	        DMSG(0, "ClientNNTP: %s group retention: %s %ld > %ld... cannot find FE",
			m_pNNTP->GetConnInfo(), grp, minrange, range, retry+1);
