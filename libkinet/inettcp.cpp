...
InetTCP::SendRecv (const char * sbuf, int slen, char * rbuf, int rlen)
{
	...
	for(i = n = 0; i < slen; i += n)
	{
		...
		{
			DMSG(0, "TCP SendRecv error: %s",
				strerror(n ? errno : (errno = EPIPE)));
			return n;
		}
	}
	...
	if(i == 0)
	{
		DMSG(0, "InetTCP: Server timeout: %s", strerror(errno=EPIPE));
		return -1;
	}
	...
	n = Recv(rbuf, rlen);
	if(n < 0) 
		DMSG(0, "InetTCP: receive error: %s", strerror(errno));
	return n;
}
