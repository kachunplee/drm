...
ostream& MailTo::OutHeader (ostream& stm)
{
	...
	stm <<
		...
		"<input type=text name=mailto id=\"Tofield\" "
		"onkeyup=\"showSuggestion(DIR_HT, this.value)\" onfocusout=\"return false;\" "
		"size=45 value=\""
	;
	...
	{
		stm << " value=\"";
		if(strncasecmp(m_pSubject, "Re:", 3) != 0)
			stm << "Re: ";
		stm << HTMLText(m_pSubject) << "\"";
	}
	stm <<
		" onfocus=\"hideSuggestion()\"></td>\n"
		"</tr>\n"
		...
		"<b>&nbsp;Cc:</b></font></td>\n"
		"<td bgcolor=#EAEFF8 height=20>"
		"<input type=text name=cc size=45 onfocus=\"hideSuggestion()\"></td>\n"
		"</tr>\n"
		"<tr>\n"
		"<td bgcolor=#EAEFF8 width=80 height=20>"
		"<font face=Arial size=2 color=#396BB5>"
		"<b>&nbsp;Bcc:</b></font></td>\n"
		"<td bgcolor=#EAEFF8 height=20>"
		"<input type=text name=bcc size=45 onfocus=\"hideSuggestion()\">"
	...
	for (int i = 0; i < 20; i++)
	{
		...
		"Repeat fro multiple attachments</font></td>\n"
		"</tr>\n"
	    ;
	}
	...
	stm <<
		...
		"<iframe frameborder=0 id=InputText name=DRMComposeBody "
		"marginheight=0 marginwidth=0 src=\"" << MAILBIN
	    << "compose?-b+"
	    << getenv("QUERY_STRING") << "\" width=670 height=360 "
		"scrolling=auto>"
		"</iframe>\n"
	;
...
ostream& MailTo::OutBody (ostream& stm)
{
	...
	stm <<
		"</head>\n"
		"<body>\n"
		"<form name=ComposeBodyForm action=" << MAILBIN << "spellchk "
		"method=POST>\n"
	;
	if (m_bHTML)
		stm <<
			"<input type=hidden name=MailHTML value='yes'>\n"
			"<textarea id=MailBody name=MailBody rows=10 cols=70 "
			"wrap=PHYSICAL"
		...
		stm <<
			"<input type=hidden name=MailHTML value=''>\n"
			"<textarea id=MailBody name=MailBody rows=20 cols=80 "
			"wrap=PHYSICAL>\n"
		;
	...
	else if(m_pOpt->GetMailType() == M_FORWARD)
	{
		if (m_pMsgInfo)
		{
			...
			stm << ">Subject: " << HTMLText(m_pSubject) << "\r>\r";
		    }
		    PrintBody(stm, *m_pMsgInfo->msgMsg);
		}
	}
	...
	if (m_bHTML)
	{
		...
		stm << "<script>\n"
		       "var ostg = \"<div><br>\\n<br>\\n</div><hr><!-- DRM QUOTE -->\\n\";\n"
		       "ostg += \"<b>From:</b> " << HTMLText(m_pFrom) << "<br>\\r\";\n"
		       "ostg += \"<b>Sent:</b> " << HTMLText(dateBuf) << "<br>\\r\";\n"
		       "ostg += \"<b>To:</b> " << HTMLText(m_pTo) << "<br>\\r\";\n"
		       "ostg += \"<b>Subject:</b> " << HTMLText(m_pSubject) << "<br>\\r\";\n"
		       "ostg += \"<p>\\r\";\n"
		       "ostg += document.getElementById('MailBody').value;\n"
		       "writeRichText('rte1', ostg, 640, 340, false, false);\n"
		       "</script>\n"
		       "</form>\n"
		       "</div>\n"
		;
