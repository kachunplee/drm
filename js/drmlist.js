...
DRMList.display = function ()
{
	if(Ajax.checkReadyState('loading') == "OK")
	{
		...
		// get obj 
		var read = nummail - unread;
		if(fd_read[currentFolder] != read || fd_unread[currentFolder] != unread)
			updateFolderCnt (currentFolder, read, unread);
		if (statusonly || currentFolder == 'in')
		{
			var obj = document.getElementById('fstat_in');
			obj.title = nummail-unread + " read " +
				unread + " unread";
		}
		if (statusonly)
		{
...
function updateFolderCnt (folder, nRead, nUnread)
{
	fd_read[folder] = nRead;
	fd_unread[folder] = nUnread;
	var eTitle = document.getElementById("drm_list_title");
	eTitle.innerHTML = folder.substr(0, 1).toUpperCase() + folder.substr(1)
		+ ' Box: <span class=drm_list_hdr style="font-weight:normal;">' + nRead
		+ ' read, ' + nUnread + ' unread</span>';
}

function displayList (folder, sort, timer)
{
	stopAutoCheck();

	statusonly = timer ? (currentFolder != 'in') : false;
	if (statusonly)
	{
		sort = '';
	}
	else
	{
		...
		var nRead = fd_read[folder];
		var nUnread = fd_unread[folder];
		var eTitle = document.getElementById("drm_list_title");
		eTitle.innerHTML = folder.substr(0, 1).toUpperCase() + folder.substr(1)
			+ ' Box: <span class=drm_list_hdr style="font-weight:normal;">' + nRead
			+ ' read, ' + nUnread + ' unread</span>';

		sort = (sort && (sort.length > 0)) ? '+-s' + sort : '';
		DRMList.init();
	}
